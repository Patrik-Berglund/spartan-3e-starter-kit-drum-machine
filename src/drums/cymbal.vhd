...
architecture rtl of cymbal is
  ...
  -- TONE: LP shift 3 (darkest) / 2 (mid, matches CY5050 reference) / 1 (brightest)
  signal lp_shift : integer range 1 to 3;
  -- DECAY: K value 11-15. decay=0->11(short), decay=255->15(long)
  signal decay_k : integer range 11 to 15;
begin
  -- Map tone 0-255 to lp_shift: low tone = darker, high tone = brighter
  lp_shift <= 3 when tone < 86 else
              2 when tone < 171 else
              1;
    ...
    if rising_edge(clk) then
      if rst = '1' then
        ...
        if sample_tick = '1' and active = '1' then
          ...
          case lp_shift is
            when 3 => lp_acc <= lp_acc + shift_right(raw - lp_acc, 3);
